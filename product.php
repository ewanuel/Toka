...
?>
    ...
    <?php
    $sql_comentarios = "SELECT usuario, comentario, data_postagem FROM comentarios WHERE produto_id = ?";
    ...
    if ($stmt_comentarios) {
        ...
        while ($row_comentario = $result_comentarios->fetch_assoc()) {
            ...
            echo '<p>' . $row_comentario['usuario'] . ' - ' . $row_comentario['data_postagem'] . '</p>';
    ...
    <!-- Formulário para postar novos comentários -->
    <div class="esc">
        <p>Escreva seu comentário</p>
        <form method="post" action="">
            <textarea name="comentario" class="esc_u" rows="4" placeholder="Digite seu comentário"></textarea>
            <input type="submit" name="postar_comentario" value="Enviar">
        </form>
    </div>

    <?php
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['postar_comentario'])) {
    $usuario = "Nome do Usuário"; // Altere para o nome do usuário real
    $comentario = $_POST['comentario'];

    $stmt_insert_comentario = $conn->prepare("INSERT INTO comentarios (produto_id, usuario, comentario) VALUES (?, ?, ?)");

    if ($stmt_insert_comentario) {
        $stmt_insert_comentario->bind_param("iss", $produto_id, $usuario, $comentario);

        if ($stmt_insert_comentario->execute()) {
            echo "Comentário postado com sucesso!";
            // Atualizar a página após a postagem do comentário
        } else {
            echo "Erro ao postar o comentário: " . $stmt_insert_comentario->error;
        }
